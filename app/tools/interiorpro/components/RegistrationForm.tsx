...
function TileGridMotif({ size = 56 }: { size?: number }) {
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} aria-hidden="true">
      ...
      {/* Grout joint markers */}
      <line x1={2 * cellSize} y1={0} x2={2 * cellSize} y2={size} stroke="rgba(255,255,255,0.25)" strokeWidth="0.4" opacity="0.2" />
      <line x1={0} y1={2 * cellSize} x2={size} y2={2 * cellSize} stroke="rgba(255,255,255,0.25)" strokeWidth="0.4" opacity="0.2" />
    </svg>
  )
}
...
export default function RegistrationForm({ onSubmit }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      onSubmit(submitData as InteriorRegData, json.contactId)
    } catch (err: unknown) {
