...
const InteriorMassingPreview3DWrapper = dynamic(
...
export default function ResultsPage({ result, input, estimateId, contactName, onStartOver }: Props) {
  ...
  const interior3DFloors = useMemo((): FloorDatum[] => {
  ...
  async function handleUnlock() {
    ...
    try {
      const res = await fetch('/api/payments/create-order', {
        ...
        body: JSON.stringify({ estimateId, amount: 89900 }),
      })
      const json = await res.json()
    ...
    <div className="min-h-screen  pb-16">
      ...
      <div className="px-6 md:px-12 lg:px-16 pt-6 space-y-6">
        ...
        <div className="border rounded-[2px]" style={{ borderColor: 'rgba(255,255,255,0.10)' }}>
            ...
            <div>
              ...
              {!show3D && (
                <p className="text-[11px] mt-0.5" style={{ color: 'rgba(255,255,255,0.35)', fontFamily: 'var(--font-plex-sans)' }}>
                  Floor massing with finish grade tinting — drag to rotate, scroll to zoom
                </p>
              )}
            </div>
            <button
          ...
          {show3D && (
            <div>
              <InteriorMassingPreview3DWrapper
                floors={interior3DFloors}
                grade={input.grade}
              />
              <div
                className="px-4 py-2"
              ...
              >
                <p style={{ fontFamily: 'var(--font-plex-mono)', fontSize: 10, color: 'rgba(255,255,255,0.30)', letterSpacing: '0.04em' }}>
                  ROUGH MASSING MODEL — tan plane on each floor = finish grade ({input.grade.toUpperCase()}) · basic=light tan · standard=mid tan · premium=brown · luxury=warm gold
                </p>
              </div>
            </div>
          )}
