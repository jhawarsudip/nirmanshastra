...
const ElectroMassingPreview3DWrapper = dynamic(
...
export default function ResultsPage({ result, input, estimateId, contactName, onStartOver }: Props) {
  ...
  const electro3DFloors = useMemo((): FloorDatum[] => {
  ...
  const downloadPdf = useCallback(async () => {
    ...
    try {
      ...
      a.href = url
      a.download = 'NirmanShastra-ElectricalPro-Report.pdf'
  ...
  async function handleUnlock() {
    ...
    try {
      ...
      const options = {
        ...
        name:        'NirmanShastra',
        description: 'ElectricalPro Report — Phase 3 Electrical',
    ...
    <div className="min-h-screen  pb-16">
      ...
      <div className="px-6 md:px-12 lg:px-16 pt-6 space-y-6">
              ...
              <tbody>
                {r.dbPanelSchedule.circuits.map((circuit, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.02)' }}>
                    <td className="py-2 pr-2" style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-plex-sans)' }}>
                      {circuit.label}
                    </td>
                    <td className="py-2 text-center font-bold" style={{ color: '#1F4E79', fontFamily: 'var(--font-plex-mono)' }}>
                      {circuit.ways}
                    </td>
                    <td className="py-2 text-center" style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-plex-mono)' }}>
                      {circuit.mcbRating}
                    </td>
                    <td className="py-2 text-center" style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-plex-mono)' }}>
                      {circuit.wireSize}
                    </td>
                    <td className="py-2 text-center text-[10px]" style={{ color: 'rgba(255,255,255,0.40)', fontFamily: 'var(--font-plex-mono)' }}>
                      {circuit.isCode}
                    </td>
                  </tr>
                ))}
                <tr style={{ borderTop: '2px solid rgba(255,255,255,0.08)' }}>
                  <td className="py-2 font-semibold" style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-plex-sans)' }}>
        ...
        <div className="border rounded-[2px]" style={{ borderColor: 'rgba(255,255,255,0.10)' }}>
            ...
            <div>
              ...
              {!show3D && (
                <p className="text-[11px] mt-0.5" style={{ color: 'rgba(255,255,255,0.35)', fontFamily: 'var(--font-plex-sans)' }}>
                  Floor massing with DB panel indicator — drag to rotate, scroll to zoom
                </p>
              )}
            </div>
            <button
          ...
          {show3D && (
            <div>
              <ElectroMassingPreview3DWrapper
                floors={electro3DFloors}
                dbPanelPerFloor={input.dbPanelPerFloor ?? false}
              />
              <div
                className="px-4 py-2"
              ...
              >
                <p style={{ fontFamily: 'var(--font-plex-mono)', fontSize: 10, color: 'rgba(255,255,255,0.30)', letterSpacing: '0.04em' }}>
                  ROUGH MASSING MODEL — DB panel position approximated at east wall corner · exact wall placement not tracked by engine · Marking Yellow = DB panel + conduit stub
                </p>
              </div>
            </div>
          )}
              ...
              <table className="w-full text-[13px]" style={{ borderCollapse: 'collapse', tableLayout: 'fixed' }}>
                ...
                <tbody>
                  <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    <td className="py-2" style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-plex-sans)', fontSize: 13 }}>Wire — 1.5 sqmm FR (lighting circuits, IS 732:2019)</td>
                    <td className="py-2 text-right" style={{ color: 'rgba(255,255,255,0.50)', fontFamily: 'var(--font-plex-mono)', fontSize: 12 }}>m</td>
                    <td className="py-2 text-right" style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-plex-mono)', fontSize: 13, fontWeight: 500 }}>{r.wireSchedule.size_1_5_m.toLocaleString('en-IN')}</td>
                  ...
                  <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.05)', background: 'rgba(30,34,39,0.018)' }}>
                    <td className="py-2" style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-plex-sans)', fontSize: 13 }}>Wire — 2.5 sqmm FR (power sockets, IS 732:2019)</td>
                    <td className="py-2 text-right" style={{ color: 'rgba(255,255,255,0.50)', fontFamily: 'var(--font-plex-mono)', fontSize: 12 }}>m</td>
                    <td className="py-2 text-right" style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-plex-mono)', fontSize: 13, fontWeight: 500 }}>{r.wireSchedule.size_2_5_m.toLocaleString('en-IN')}</td>
                    <td className="py-2 text-right" style={{ color: 'rgba(255,255,255,0.50)', fontFamily: 'var(--font-plex-mono)', fontSize: 13 }}>—</td>
                  </tr>
                  <tr>
                ...
                <tbody>
                  {[
                    ...
                    <tr key={i} style={{ borderBottom: '1px solid rgba(30,34,39,0.06)', background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.02)' }}>
                      ...
                      <td className="py-2 text-right font-bold" style={{ color: 'var(--text-primary)', fontFamily: 'var(--font-plex-mono)' }}>
                        {row.qty}
                      </td>
                  ...
                  <tr style={{ borderTop: '2px solid rgba(255,255,255,0.20)' }}>
                    <td className="py-2 font-semibold" style={{ fontFamily: 'var(--font-plex-sans)', color: 'var(--text-primary)' }}>
                      Total Points
                    </td>
